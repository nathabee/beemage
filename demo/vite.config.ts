  ...
  return {
    ...
    async resolveId(source, importer, options) {
      ...
      if (isPanelRuntime(id)) {
        // Useful when debugging “why is demo still compiling extension runtime?”
...
function ensureOpenCvPublicPlugin(): Plugin {
  ...
  function ensureOpenCv(): void {
    ...
    // Prefer committed runtime (good for reproducible builds / CI / GitHub Pages), fallback to node_modules.
