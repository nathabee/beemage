...
[[ -f ".fdroid.yml" ]] || die ".fdroid.yml not found at repo root"
...
# Guard (prepare-step): only refuse generated output paths.
...
[[ -z "$forbidden" ]] || {
  echo "Generated build/publish outputs detected (do not include these in the release commit):"
  echo "$forbidden"
...
# Update .fdroid.yml to match current VERSION + versionCode (release-prep behavior)
python3 - <<PY
import pathlib, re, sys

path = pathlib.Path(".fdroid.yml")
...
if missing:
  sys.stderr.write("ERROR: .fdroid.yml is missing required fields for auto-update:\n")
  for k in missing:
