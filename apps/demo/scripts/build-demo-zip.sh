...
# 2) Sanity check
[[ -d "$DEMO_DIR/dist" ]] || { echo "Missing apps/demo/dist/ (build failed?)"; exit 1; }
[[ -f "$DEMO_DIR/dist/index.html" ]] || { echo "Missing apps/demo/dist/index.html"; exit 1; }
...
# 3) Create zip (always into repo-root release/)
OUT_DIR="${ROOT_DIR}/release"
mkdir -p "$OUT_DIR"

ZIP_PATH="${OUT_DIR}/beemage-demo-${ver}.zip"
rm -f "$ZIP_PATH"

(
  cd "$DEMO_DIR/dist"
  zip -qr "$ZIP_PATH" . -x "*.map"
)
